...
Another useful testing target is `smith fontspector`. This runs all the generated fonts through the https://github.com/fonttools/fontspector/[Fontspector] QA suite (which is the reimplementation of Font Bakery in Rust).
It does so using the silfonts.toml profile which explicitly list certains checks, excludes others, overrides and provides new ones. 
A html report is generated with the results for each font family along with a summary at the command-line.
